...
public class ActorController {
    ...
    public ResponseEntity<ApiResponse<PageResponse<ActorResponse>>> getAllPaged(
            @Parameter(description = "Số trang (bắt đầu từ 0)", example = "0")
            @RequestParam(defaultValue = "0") int page,
            @Parameter(description = "Số lượng mỗi trang", example = "10")
            @RequestParam(defaultValue = "10") int size) {
        return ResponseEntity.ok(ApiResponse.success(actorService.getAllPaged(page, size)));
    }
    ...
    public ResponseEntity<ApiResponse<PageResponse<ActorResponse>>> search(
            ...
            @Parameter(description = "Số lượng mỗi trang", example = "10")
            @RequestParam(defaultValue = "10") int size) {
        return ResponseEntity.ok(ApiResponse.success(actorService.search(name, page, size)));
    }
}
